    ...
    <v-dialog v-model="dialog2" max-width="1000px">
      <v-card class="mx-auto  mt-4" max-width="700">
        ...
        <v-card-text class="p-0 py-0 px-7  mb-2">
          ...
          <v-row class="p-0 py-4 px-0  ">
            <v-col class="p-0 py-0 px-1">
              <v-textarea v-model="seguimiento.observacion" :readonly="lockField2" label="Observación a la denuncia"
                :rules="[v => !!v || 'Observación es requerida']" placeholder="Observaciones/recomendaciones"></v-textarea>
            </v-col>
          </v-row>
          <v-row>
      ...
      Actividad.actividadList()
        .then((response) => {
          ...
          if (response.status === 200) {
           ...
           // }
           // else{
             //RBC 18 this.actividadOptions = this.actividadOptions.filter(elemento => !elemento.actividad.startsWith('DENUNCIA'));
              this.actividadOptions = this.actividadOptions.filter(elemento => elemento.tipo.startsWith('SEGUIMIENTO'));
           // }
